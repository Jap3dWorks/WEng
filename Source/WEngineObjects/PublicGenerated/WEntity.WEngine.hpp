// [[file:../WEngineObjects.org::WEntity-wengine-hpp][WEntity-wengine-hpp]]
#pragma once
...
#define WOBJECT_BODY                               \
    public:                                        \
    constexpr WEntity() noexcept = default;           \
    constexpr WEntity(const WEntity &) = default;        \
    constexpr WEntity(WEntity &&) noexcept=default;      \
    WEntity & operator=(const WEntity &) = default;      \
    WEntity & operator=(WEntity &&) = default;           \
    ~WEntity() override = default;                    \
    static const WClass * StaticClass() noexcept ; \
    const WClass * Class() const override {        \
        return WEntity::StaticClass();                \
    }
// WEntity-wengine-hpp ends here
