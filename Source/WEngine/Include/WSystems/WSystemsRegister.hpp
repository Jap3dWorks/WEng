...
public:
    ...
    wid::WSystemId RegSystem(const char * in_name, const WSystemFn & in_system);

    WSystemFn Get(const wid::WSystemId & in_id) const {
        return system_set_.Get(in_id.GetId());
    }

    wid::WSystemId GetId(const char * in_name) const {
        return name_wid_.at(in_name);
    }

    wid::WSystemId GetId(std::string_view in_name) const {
...
private:
    ...
    wcr::IdPool<wid::WSystemId::IdType> id_pool_;
    std::unordered_map<std::string, wid::WSystemId> name_wid_;
