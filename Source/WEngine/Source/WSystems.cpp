...
START_DEFINE_WSYSTEM(System_InitCameraInput)
    ...
    parameters.engine->InputMappingRegister().BindAction(
        ...
        [cid](const WInputValuesStruct & _v, const WActionStruct & _a, WEngine * _e) {
            ...
            transform.position -= glm::vec3{transform.transform[2]} * 0.01f;
    ...
    parameters.engine->InputMappingRegister().BindAction(
        ...
        [cid](const WInputValuesStruct & _v, const WActionStruct & _a, WEngine * _e) {
            ...
            transform.position -= glm::vec3{transform.transform[0]} * 0.01f;
            transform.transform[3] = {transform.position, 1};
        }
        );
    ...
    // Init camera transform matrix, TODO Move to another place.
    WTransformStruct & t = parameters.engine
        ->LevelInfo()
        .level.GetComponent<WTransformComponent>(cid)->TransformStruct();
