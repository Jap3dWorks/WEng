...
namespace wvk::raii::descriptors {

    template<std::uint8_t FramesInFlight>
    class PostprocessGlobalDesc {
    public:
    ...
    public:    
        ...
        PostprocessGlobalDesc() = default;

        PostprocessGlobalDesc(
            const PostprocessGlobalDesc &
            ) = delete;
        PostprocessGlobalDesc& operator=(
            const PostprocessGlobalDesc &
            ) = delete;

        PostprocessGlobalDesc(PostprocessGlobalDesc && other) noexcept :
        ...
        PostprocessGlobalDesc& operator=(PostprocessGlobalDesc && other) noexcept {
            if(this != &other) {
                Destroy();
        ...
        virtual ~PostprocessGlobalDesc() {
            Destroy();
        }

        PostprocessGlobalDesc(VkDevice in_device) :
            device_(in_device)
            {
                Initialize(device_);
            }
